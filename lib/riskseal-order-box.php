...
/**
 * WC_RiskSeal_OrderDetails_Box class
 * Displays RiskSeal order verification status on the WooCommerce Order Detail page
 */
class WC_RiskSeal_OrderDetails_Box {

	public static function init() {
		add_action( 'add_meta_boxes', __CLASS__ . '::add_riskseal_score_meta_box' );
	}

	public static function add_riskseal_score_meta_box() {
		add_meta_box(
			'order_score_meta_box',								// Meta box ID.
			'RiskSeal Score',									// Title.
			__CLASS__ . '::display_riskseal_score_meta_box',	// Display function.
	...
	public static function display_riskseal_score_meta_box( $post ) {
		$order_id = $post->ID;
		$status    = get_post_meta( $order_id, '_riskseal_status', true );
		$score    = get_post_meta( $order_id, '_riskseal_score', true );
		$check_id    = get_post_meta( $order_id, '_riskseal_check_id', true );
		...
		?>
		<p>Order Score: <?php echo esc_html( $score ); ?></p>
		<p>Status: <?php echo esc_html( ucfirst($status) ); ?>
		<?php
		switch ($status) {
			case 'fail':
				?>
	...
	<p>Details: <a href="<?php echo esc_url_raw(RISKSEAL_CLIENT_PORTAL_URL . 'checks/' . $check_id); ?>" target="_blank"><?php echo esc_html( $check_id ); ?></a></p>
	<?php
	}
}
